...
def read_pic_to_excel(line, excel_path, excel_result, txt_result):
    ...
    app.quit()
    print("写入excel表格完成")
    # 第七步把后半部分的字母修改成数字，生成txt文件
    number_part = re.sub(r"[o|O]",'0',number_part)
    letter_part_head = re.sub(r"0","O",letter_part[:2])
    end_part = re.sub(r"[o|O]","0",letter_part[2:])
    new_line = number_part + letter_part_head + end_part
    with open("{}/{}.txt".format(txt_result, new_line), "a", encoding='utf8') as f:
        f.write(new_line)
    print("写入txt完成")
