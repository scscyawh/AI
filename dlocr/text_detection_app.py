...
def drop_char_chiese(texts):
    result = []
    for i in texts:
        if re.compile(r'\d+').search(i) is not None:
            result.append(i)
            continue
        if re.compile(r'[a-zA-Z]+').search(i) is not None:
            result.append(i)
            continue
        if re.compile(r'-').search(i) is not None:
            result.append(i)
            continue
    return ''.join(result)
...
class TextDetectionApp:
    ...
    def detect(self, image, adjust=True, parallel=True):
        ...
        # 返回 1.所有文字框的坐标信息  2.原图像
        ctpn_time = time.time()
        with self.graph.as_default():
            text_recs, img = self.ctpn.predict(image, mode=2)
            print("cptn耗费时间:{:0.2f}s".format(time.time() - ctpn_time))
            if len(text_recs) == 0:
                return None, None
            # 对文字框的坐标信息进行排序
        ...
        if parallel:
            # 根据所有文字框的坐标信息进行截图,imgs是列表，元素为图片
            ...
            for im in imgs:
                ...
                # 开始对所有截图的文字框依此进行识别
                with self.graph.as_default():
                    texts = self.ocr.predict_multi(imgs,
                                                   id_to_char=self.id_to_char)
                    texts = texts[0]
                    texts = drop_char_chiese(texts)
                    # 开始筛选，符合要求结束识别。
                    p3 = r'.+-.+-.+-.+'  # ***-***-***-***
                    resultp3 = re.compile(pattern=p3).search(texts)
                    if resultp3 is not None:
                        continue
                    p2 = r'.+-.+-.+'  # ***-***-***
                    resultp2 = re.compile(pattern=p2).search(texts)
                    if resultp2 is not None:
                        continue
                    p1 = r'\d+[a-zA-Z]*-[a-zA-Z]*\d*'  # 数字**-**数字
                    result = re.compile(pattern=p1).search(texts)
            ...
            print("ocr耗费时间:{:0.2f}s".format(time.time() - ocr_start))
